...
/* 36 k seems to be a magic number that makes lucid happy */
#define N_SEG 64
unsigned char buf[1024 * N_SEG];	/* size should depend on sample_rate */
...
/* If lucid allowed to use its own sigint handler, then the audio */
  /* device does not get closed correctly */
  /* close device before returning under all conditions and */
